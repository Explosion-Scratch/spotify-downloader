...
app.get("/downloadSong", async (req, res) => {
  ...
  var cmd = `ffmpeg -i output.mp3 -i cover.png -c:a copy -c:v copy -map 0:0 -map 1:0 -id3v2_version 3 -metadata:s:v title="Album cover" -metadata:s:v comment="Cover (front)" cover.mp3`;
  ffmpeg("output.mp3")
    .outputOptions()
    .save("cover.mp3")
    .on("start", function (cmdline) {
