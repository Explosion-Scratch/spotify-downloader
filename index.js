...
app.get("/downloadSong", async (req, res) => {
  var id = short("abcdefghijklmnopABCDEFGHIJKLMNOP_").new();
  id = `_${id}`;
  ...
  const command = `-i output${id}.mp3 -i cover${id}.png -c:a copy -c:v copy -map 0:0 -map 1:0 -id3v2_version 3 -metadata:s:v title="Album cover" -metadata:s:v comment="Cover (front)" audio_out${id}.mp3`;
  ...
  run(
    ...
    () => {
      ...
      res.end(fs.readFileSync(`audio_out${id}.mp3`));
      del();
    }
  );
});
